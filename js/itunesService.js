angular.module('itunes').service('itunesService', function($http, $q){
  ...
  //You can return the http request or you can make your own promise in order to manipulate the data before you resolve it.
    ...
    //Code here
     ...
     this.getArtist = (artist) => $http.jsonp('https://itunes.apple.com/search?term=' + artist + '&callback=JSON_CALLBACK').then(function(response){
       console.log(response);
       let mainArr = response.data.results.map(function(album){
         return {
           Play: album.previewUrl,
           Artist: album.artistName,
           Song: album.trackName,
           Collection: album.collectionName,
           AlbumArt: album.artworkUrl60,
           Type: album.kind,
